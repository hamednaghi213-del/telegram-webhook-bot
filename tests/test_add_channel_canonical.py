...
class Query:
    ...
    def upsert(self, payload, on_conflict=None):
        ...
        assert on_conflict == "workspace_id,destination_id"
        return self

    def execute(self):
        rows = self.client.rows[self.table]
        matching = [row for row in rows if all(
            (row.get(key) == value if op == "eq" else
             row.get(key) != value if op == "neq" else
             row.get(key) is None)
            for op, key, value in self.filters
        )]
        if self.action == "select":
            return types.SimpleNamespace(data=[dict(row) for row in matching[:self.maximum]])
        if self.action == "insert":
            ...
            if self.table == "publication_destinations":
                ...
                for row in rows:
                    if (row["platform"], row.get("normalized_external_id")) == (
                        payload["platform"], payload.get("normalized_external_id")
        ...
        if self.action == "update":
            ...
            return types.SimpleNamespace(data=[dict(row) for row in matching])
        if self.action == "upsert":
            payload = dict(self.payload)
            if any(row["destination_id"] == payload["destination_id"]
                   and row["workspace_id"] != payload["workspace_id"]
                   and row["status"] == "active" for row in rows):
                raise RuntimeError("destination already has an active workspace")
            existing = next((row for row in rows
                             if row["destination_id"] == payload["destination_id"]
                             and row["workspace_id"] == payload["workspace_id"]), None)
            if existing:
...
class Client:
    def __init__(self):
        self.rows = {"publication_destinations": [], "workspace_destinations": []}
...
def database(monkeypatch):
    ...
    spec = importlib.util.spec_from_file_location("add_channel_database_under_test", path)
...
def test_unassociated_destination_keeps_media_identity_and_other_owner_rejected(database):
    module, client = database
    client.rows["publication_destinations"].append({
        "id": 7, "workspace_id": 1, "platform": "telegram",
        "external_id": "@Existing", "normalized_external_id": "existing",
        "media_identity_id": 45, "status": "inactive",
    })
    destination, outcome = module.register_setup_destination_canonical(
        2, "telegram", "@EXISTING", "Other name"
    )
    assert outcome == "associated"
    assert destination["id"] == 7 and destination["media_identity_id"] == 45
    ...
    assert client.rows["workspace_destinations"] == [{
        "workspace_id": 2, "destination_id": 7,
        "status": "active", "updated_at": client.rows["workspace_destinations"][0]["updated_at"],
    }]
...
def test_legacy_null_identity_and_competing_insert_do_not_duplicate(database):
    module, client = database
    client.rows["publication_destinations"].append({
        "id": 8, "workspace_id": 1, "platform": "bale",
        "external_id": "@Old", "normalized_external_id": None,
        "media_identity_id": 55, "status": "inactive",
    })
    reused, outcome = module.register_setup_destination_canonical(
        1, "bale", "@OLD", "Old"
    )
    assert outcome == "associated" and reused["id"] == 8
    assert client.rows["publication_destinations"][0]["normalized_external_id"] == "old"

    client.racing_row = {
        "id": 9, "workspace_id": 2, "platform": "telegram",
        "external_id": "@Race", "normalized_external_id": "race",
        "media_identity_id": None, "status": "inactive",
    }
    raced, race_outcome = module.register_setup_destination_canonical(
        2, "telegram", "@RACE", "Race"
    )
    assert raced["id"] == 9 and race_outcome == "associated"
    assert len(client.rows["publication_destinations"]) == 2
